...
predict(lm_occupancy,tibble(ADR = c(0.7)))[[1]]

tibble(room_rate = c(123, 123*1.7), occupancy = c(76, 76*(1+test))) %>%
  mutate(oktoberfest = F)

tibble(room_rate = c(401, 401*1.7), occupancy = c(100, 100*(1+test))) %>%
  mutate(oktoberfest = T) %>%
  rbind(tibble(room_rate = c(123, 123*1.7), occupancy = c(76, 76*(1+test))) %>%
  mutate(oktoberfest = F)) %>%
  ggplot(aes(x = room_rate, y = occupancy, colour = oktoberfest)) +
  geom_line()
...
A linear regression analysis was conducted to create a relationship between price and occupancy rate in the Munich hotel market. The relative change in occupancy was defined as the dependent variable, while the relative difference in average daily rate was to be considered as the explanatory variable.

```{r}
lm_occupancy <- filter(revpar_and_occupancy, kpi == "Occupancy") %>%
  lm(formula = rel_change ~ ADR, data = .)

create_occ_rates <- function(def_room_rate, def_occupancy) {
  rel_adr_change = seq(-0.5,0.5,by=0.05)
  values <- tibble(
  ADR = numeric(),
  occupancy = numeric())
  
  for (adr in rel_adr_change) {
    values <- tibble(ADR = adr) %>%
      mutate(occupancy = adr*lm_occupancy$coefficients[[2]]) %>%
      mutate(ADR = (ADR + 1)*def_room_rate,
             occupancy = (occupancy+1)*def_occupancy) %>%
      rbind(values,.)
  }
  return(values)
}
...
rates1 <- create_occ_rates(123,70) %>%
  mutate(rates = 1)

rates2 <- create_occ_rates(400,100) %>%
  mutate(rates = 2)

rbind(rates1,rates2) %>%
  ggplot(aes(x = ADR, y = occupancy), colour = rates) +
  geom_point()
